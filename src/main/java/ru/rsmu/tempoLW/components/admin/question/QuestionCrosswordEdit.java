 ...
 * @author leonid.
 */
public class QuestionCrosswordEdit {
    ...
    private void prepare() {
        ...
        for ( CorrespondenceVariant correspondenceVariant : getQuestionCrossword().getCorrespondenceVariants() ) {
    ...
    public ValueEncoder<CorrespondenceVariant> getCorrespondenceVariantEncoder() {
        return new ValueEncoder<CorrespondenceVariant>() {
            ...
            public CorrespondenceVariant toValue( String clientValue ) {
                ...
                for ( CorrespondenceVariant variant : getQuestionCrossword().getCorrespondenceVariants() ) {
                    if ( variant.getId() == id ) {
                        return variant;
                    }
                }
    ...
    public QuestionCrossword getQuestionCrossword() {
        return (QuestionCrossword) question;
    }
    ...
    public void onDeleteVariant( CorrespondenceVariant variant ) {
        ...
        getQuestionCrossword().getCorrespondenceVariants().remove( variant );
        variant.getCorrectAnswers().clear();
    ...
    public Object onSuccess() {
        ...
        if ( StringUtils.isNotBlank( additionalCorrespondenceText ) ) {
            ...
            variant.setQuestion( question );
            getQuestionCrossword().getCorrespondenceVariants().add( variant );
            applySubmit = true;
        }
        questionDao.save( question );
        if ( applySubmit ) {
