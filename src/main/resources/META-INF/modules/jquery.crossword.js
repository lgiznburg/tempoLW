			...
			*/
			...
			var puzInit = {
				...
				init: function() {
					...
					// this code was inside commented functions -- L.G.
					util.highlightEntry();
					util.highlightClue();
					$('.active').eq(0).focus();
					$('.active').eq(0).select();
				...
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							if($(light).empty()){
								$(light)
									.addClass('entry-' + (x - currentOffset) /!*(hasOffset ? x - positionOffset : x)*!/ + ' position-' + (x-1) + ' puzzle-cell' )
...
*/
				...
				*/
				checkAnswer: function(e) {
					...
					let currentEls = $('.position-' + activePosition + ' input');

					currVal = currentEls/*$('.position-' + activePosition + ' input')*/
						.map(function() {
					  		return $(this)
								.val()
								.toLowerCase();
					...
					// save value into input if all cells is filled
					if ( /*$('.position-' + activePosition + ' input')*/currentEls.length === currVal.length ) {
						$('.input-element' + activePosition).val( currVal );
					}
			...
			var nav = {
				...
				updateByNav: function(e) {
					...
					//activeClueIndex = $(clueLiEls).index(e.target);  old version
				...
				updateByEntry: function(e, next) {
					...
					if(e.keyCode === 9 || next){
						...
						//clue = $(clueLiEls + '[data-position=' + activePosition + ']');  old version
						clue = clueLiEls.filter('[data-position=' + activePosition + ']');
						//activeClueIndex = $(clueLiEls).index(clue);  old version
			...
			var util = {
				...
				highlightClue: function() {
					var clue;				
					$('.clues-active').removeClass('clues-active');
					//$(clueLiEls + '[data-position=' + activePosition + ']').addClass('clues-active');  old version
				...
				getClasses: function(light, type) {
					...
					//var classes = $(light).prop('class').split(' '),  old version
				...
				getActivePositionFromClassGroup: function(el){
					...
					let classes = util.getClasses($(el).parent(), 'position');
				...
				getSkips: function(position) {
					//if ($(clueLiEls[position]).hasClass('clue-done')){  old version
